...
// > useContext를 사용하면 중간에 props전해주는 것 없이 자손컴포넌트에 값을 전해줄수있다

import React, { Component } from "react";
import { useContext } from "react";

// (1) useContext통햇 공유할 값을 객체형태로 작성
const themes = {
    light : {
        foreground : "#000000",
        background : "#eeeeee"
...
// (2) useContext를 통해서 값을 고유하기위해 createContext사용
// React를 improt해서 사용
...
// 함수형으로 만들어주셔서 동일한 theme.dark를 가질수 있도록하기
// + themes에 blue를 추가해서 배경색 blue이고 color가 white주제를 만들어서 확인
const ThemedTextFunc = () =>{
    const theme = useContext(ThemeContext);
    return (
        <div>
            <p style={{backgroundColor: theme.background, color:theme.foreground}}>다크모드로 진행하고 있습니다</p>
        </div>
    );
}