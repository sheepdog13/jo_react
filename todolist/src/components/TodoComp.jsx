...
function reducer(state, action) {
    switch (action.type) {
        ...
        case "checkedlist":
            // 리스트안에서 객체 하나를 찾아서 그 객체의 done을 !값으로 바꿔줌
        ...
        case "deletelist" :
            // id 값을 가져와서, id값과 같은 객체를 제외하고 배열 생성
...
const TodoComp = () => {
    ...
    return (
        <div>
            <input type="text" onChange={(e)=>{dispatch({type:"todoInput", payload : e.target.value})}}/>
            <button onClick={()=>{dispatch({type: "todoAdd"})}}>+</button>

            <ul>
                <li>
                    {/* TodoItem - 체크여부, 할일, id값(삭제에 필요) */}
                    <input type="checkbox" checked={state.todoitem.done}
